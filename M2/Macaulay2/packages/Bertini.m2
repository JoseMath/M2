...
importFiniteSolutions= method()
importFiniteSolutions(String,String,ZZ,Function) := (dir,fn,nc, eval) -> (
    f := s-> break (value s) ;
    ns := scanLines( f , dir|fn ) ;
    sols:= new MutableList from 0..(nc*(ns-1));
    count :=0;
    g := s-> (
	s = separate(" ",s);
	if #s==2 then (
	    s = s/(i->value concatenate separate("+",i));
	    s = toCC(toRR(s#0),toRR(s#1));
	    sols#count = s;
	    count=count+1
	    );
	);    	    	     
...
PartialMonodromySolve =new Type of MutableHashTable
setupPartialMonodromySolve :=(I,c,x)->(
    kk := coefficientRing ring I;
    if not instance(kk, PolynomialRing) then error"Coefficient ring needs to be generated by parameters.";
    if #c=!=#gens kk then error"The number parameters does not agree with the coefficient ring.";
    if #x=!=#gens ring I then error"The number of coordinates does not agree with the number of variables.";
    X:=gens ring I;
    C:=gens kk;
    dir:=temporaryFileName();
    mkdir dir;
    pwc := new PartialMonodromySolve from {
    	-1=> new MutableList from C, --parameters
    	-2=> new MutableList from X, --variables
	-3=>new MutableList from (I_*)	--polynomials
	-4=>dir
	};
    symbol tRestrict;
    S := CC[tRestrict];    
    --restriction
    pwc#-4 = new MutableList from  apply(c,gens kk,(i,j) -> j=>random CC*tRestrict+ i);--tRestrict=0 gives us c. 
    --slice to solve for generic parameters
    --0 => Slice A
    --1 => Solution A
    --2 => Scale A
    pwc#0 = random CC*tRestrict;
    pwc#1 = new MutableList from {x|{0}};
    pwc#2 = random CC;
    -- slice for trace test
    pwc#3 = sum(X,i->random(coefficientRing kk)*(i-random(coefficientRing kk)));
    pwc#4 = random CC;
    pwc#5 = new MutableList from {};
    pwc
    )
solve(ZZ,PartialMonodromySolve):=(k,pwc)->(
    if k<0 or k%3=!=0 then error"Integer needs to be mod 3 to index correctly.";
    sp := pwc#k;
    symbol tau;
    T := CC[tau];        
    dir :=pwc#-4|"/"|k;
    if not fileExists dir then mkdir;
    makeB'InputFile(dir,
	AffVariableGroup=>pwc#-2|{},
	B'Functions=>apply(pwc#-1,pwc#-4,(i,j)->i=>j),
	B'Polynomials=>pwc#-3|{pwc#k|"+"|toString(tau)},
    	BertiniInputConfiguration=>{ParameterHomotopy=>2}
	);
    writeParameterFile(dir,{0},NameParameterFile=>"start_parameters");
    delta1:=random CC;
    delta2:=pwc#(k+2);
    writeParameterFile(dir,{delta1},NameParameterFile=>"final_parameters");
    runBertini(dir);
    moveB'File(dir,"finite_solutions","start");            
    moveB'File(dir,"final_parameters","start_parameters");            
    makeB'InputFile(,
	AffVariableGroup=>pwc#-2|{},
	B'Functions=>apply(pwc#-1,pwc#-4,(i,j)->i=>j),
	B'Polynomials=>pwc#-3|{parString(delta2)|"*"|parString(pwc#k)|"+"|toString(tau)},
    	BertiniInputConfiguration=>{"ParameterHomotopy"=>2}
	);
    writeParameterFile(dir,{delta1*delta2},NameParameterFile=>"start_parameters");
    writeParameterFile(dir,{0},NameParameterFile=>"final_parameters");
    runBertini(dir);
    )
...
R=QQ[a,b][x,y]
I=ideal(a*b+y,b*y+3*x)
pwc = setupPartialMonodromySolve(I,
    {random CC,random CC},{random CC,random CC})
class pwc
solve(0,pwc)
...
-*
eval=lc->( (pack(lc,4))/sum)

getTrace=(s->(
	sols = importFiniteSolutions(dir,"finite_solutions_"|s,13,eval);
	1/#sols*sum sols
	)    )
...
avgB = getTrace"B"
avgC = getTrace"C"
VerticalList((avgA-avgB)-(avgB-avgC))
help pack
#sols
first sols
class sols
